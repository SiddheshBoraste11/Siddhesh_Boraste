

<h1 align="center"> Hi there 👋, I'm Siddhesh Boraste

<h1 align="center">Data Analyst | AI & Data Science Graduate
##
  
I'm an AI & Data Science graduate passionate about turning raw data into meaningful insights. I enjoy solving business problems through data analysis, creating interactive dashboards, and building data-driven solutions using SQL, Python, Excel, and Power BI.

---

## 🚀 About Me

* 🎓 AI & Data Science Graduate
* 📊 Aspiring Data Analyst
* 💡 Passionate about Data Analytics, Business Intelligence & Data Visualization
* 🌱 Currently learning Advanced SQL, Power BI, Statistics, and Data Warehousing
* 🎯 Interested in Data Analytics, Business Intelligence, and Decision Support Systems

* 🎓 B.E. in Artificial Intelligence & Data Science (2025)
* 📍 Pune, Maharashtra, India
* 💼 Open to Data Analyst, Business Analyst, and Junior Data Engineer opportunities

---

## 🛠️ Tech Stack

### Programming

* Python
* SQL

### Data Analysis

* Pandas
* NumPy
* Matplotlib

### Visualization

* Power BI
* Microsoft Excel

### Database

* MySQL
* SQL Server

### Concepts

* Data Cleaning
* ETL
* Data VisualizationDashboard Development
* Business Analytics

### Other Tools

* Git
* GitHub
* VS Code
* Jupyter Notebook

---

## 📂 Featured Projects

### 📊 Credit Card Customer Analysis

* Built an interactive Power BI dashboard to analyze customer behavior, spending patterns, and churn trends.
* Developed KPI cards, DAX measures, and dynamic visualizations to generate business insights.

---

### 🌾 Crop Production Analysis

* Analyzed agricultural production data across Indian states using Power BI.
* Identified regional trends, crop-wise performance, and productivity patterns through interactive dashboards.

---

### 💬 Student Feedback Sentiment Analysis

* Developed an LSTM-based sentiment analysis model using Python and NLP techniques.
* Classified student feedback into Positive, Neutral, and Negative sentiments to support educational insights.

---

## 📈 GitHub Goals

* Build real-world data analytics projects
* Improve SQL problem-solving skills
* Explore business intelligence solutions
* Contribute to open-source projects
* Continuously learn and share knowledge

---

## 📚 Currently Learning

* Advanced SQL
* Power BI (DAX & Data Modeling)
* Statistics for Data Analysis
* Data Warehousing
* ETL Concepts

---

## 📫 Connect With Me

* 💼 LinkedIn: www.linkedin.com/in/siddheshb11
* 🌐 Portfolio: https://siddhesh-boraste-portfolio.vercel.app/
* 📧 Email: siddboraste11@gmail.com

---

<h1 align="center">### ⭐ Thanks for visiting my profile! ⭐
I'm always open to learning, collaborating, and building impactful data-driven solutions.
